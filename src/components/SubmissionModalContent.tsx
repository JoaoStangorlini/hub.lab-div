...
interface SubmissionModalContentProps {
    item: MediaCardProps;
    imageIdx: number;
    setImageIdx: (idx: number | ((prev: number) => number)) => void;
    onClose: () => void;
    hasPrev: boolean;
    hasNext: boolean;
...
export function SubmissionModalContent({
    item,
    imageIdx,
    setImageIdx,
    onClose,
    ...
    const [citeCopied, setCiteCopied] = React.useState(false);
            ...
            >
                ...
                <div className="flex-1 bg-black flex items-center justify-center relative min-h-[30vh] lg:min-h-full group">
                    {item.mediaType === 'video' ? (
                        (() => {
                            const rawUrls = parseMediaUrl(item.mediaUrl);
                        ...
                        (() => {
                            ...
                            return pdfUrl ? (
                                <div className="w-full h-full min-h-[60vh] md:min-h-full bg-white rounded-l-2xl md:rounded-l-3xl overflow-hidden relative">
                                    <CustomPdfViewer fileUrl={pdfUrl} title={item.title} />
                                </div>
                            ) : (
                        ...
                        (() => {
                            const urls = parseMediaUrl(item.mediaUrl);
                            if (urls.length === 0) return <span className="text-white">Imagem não encontrada</span>;

                            return (
                                <div className="relative w-full h-full flex items-center justify-center">
                                    <img src={urls[imageIdx]} alt={item.title} className="max-w-full max-h-[80vh] object-contain" />
                                    {urls.length > 1 && (
                                        <>
                                            <button
                                                onClick={(e) => { e.stopPropagation(); setImageIdx(p => (p - 1 + urls.length) % urls.length) }}
                                            ...
                                            <button
                                                onClick={(e) => { e.stopPropagation(); setImageIdx(p => (p + 1) % urls.length) }}
                                            ...
                                            <div className="absolute bottom-6 flex gap-2 bg-black/40 px-3 py-2 rounded-full backdrop-blur-md">
                                                {urls.map((_, i) => (
                                                    <div
                                                        key={i}
                                                        className={`h-2.5 rounded-full cursor-pointer hover:bg-white transition-all ${i === imageIdx ? 'w-8 bg-white' : 'w-2.5 bg-white/50'}`}
                ...
                <div className="w-full lg:w-[400px] xl:w-[450px] shrink-0 bg-white dark:bg-gray-900 p-6 md:p-8 overflow-y-auto flex flex-col gap-6 max-h-[50vh] lg:max-h-full border-t lg:border-t-0 lg:border-l border-gray-100 dark:border-gray-800">
                    ...
                    <div className="pt-4 flex flex-col gap-3 mt-auto">
                        ...
                        {item.mediaType === 'image' && (
                            <a
                                href={getDownloadUrl(parseMediaUrl(item.mediaUrl)[imageIdx])}
                                target="_blank"
                                rel="noopener noreferrer"
